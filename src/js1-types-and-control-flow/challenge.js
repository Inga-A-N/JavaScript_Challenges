...
export const findLargestNumber = () => {
  return largeNumber1 > largeNumber2 ?
  largeNumber1:
  largeNumber2
};
...
export const getIsValidOnNameTag = () => {
  ...
  return (nameTagOption.length <= 8 &&  nameTagOption[0] === nameTagOption[0].toUpperCase())
...
export const convertPascalCaseToSnakeCase = () => {
  let newString = pascalCaseVariableName;

  if (newString[0] === newString[0].toUpperCase()){

    newString = (newString.replace(newString[0], newString[0].toLowerCase()));
  }
  for (let i = 1; i <= pascalCaseVariableName.length; i+=1){
    
    if (newString[i] === newString[i].toUpperCase()){
        
      newString = (newString.replace(newString[i], "_"+ newString[i].toLowerCase()));
    }
